# -*- coding: utf-8 -*-
"""AE-distribution-at-output.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1v5fbPTBggjeCuqRaEFWgqRMt6uHNStpG
...
for number_of_distributions, arch_type, dist_type, dataset_name in [
    [1,'mlp','normal','mnist']
    ]:
  try:
    ...
    OUTPUT_DIRECTORY = "C:\\Users\\Sebi\\Desktop\\paper-june\\results"
    ...
    def simple_autoencoder(inputs):
      ...
      if arch_type == "mlp":
        ...
        if dist_type == "matrix_mse":
          ...
          decoder = tf.keras.layers.Conv2D(filters=1, kernel_size=(3,3), activation=ACTIVATION, padding='same')(encoder)
          ...
          encoder = tf.keras.layers.Conv2D(filters=UNITS, kernel_size=(3,3), activation=ACTIVATION, padding='same')(inputs)
          encoder = tf.keras.layers.MaxPooling2D(pool_size=(2,2))(encoder)
          decoder = tf.keras.layers.Conv2D(filters=n_filters, kernel_size=(3,3), activation=ACTIVATION, padding='same')(encoder) 
          decoder = tf.keras.layers.UpSampling2D(size=(2,2))(decoder)
    ...
    if number_of_distributions == 1 and dist_type in ["normal_with_diag_cov", "normal", "t_with_diag_cov", "t", 
      ...
      def display_results(disp_input_images, disp_encoded, disp_predicted, enc_shape=(8,4)):
        ...
        f.suptitle(str(number_of_distributions) + "-" + arch_type + "-" + dist_type +"-" +dataset_name)
        f.savefig(os.path.join(OUTPUT_DIRECTORY, str(number_of_distributions) + "-" + arch_type + "-" + dist_type +"-" +dataset_name + "-plot1.pdf"))
      if dist_type in ["normal_with_diag_cov", "normal"]:
      ...
      elif dist_type in ["t_with_diag_cov", "t"]:
